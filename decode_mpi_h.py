#mpi decoding in high error rate environments
...
from LDPC_h import log_LDPC_decode
...
class Decode_mpi_unit :
    ...
    def fast_decode(self) :
        ...
        for line in tqdm.tqdm(input_file.readlines() ):
            ...
            L_int_z= zeros(self.total_msg+self.trellis.total_memory)
            #print(L_int_z.size())
            result_bcjr = log_map_decode(now_code_1,now_code_2,self.total_msg,self.trellis,self.pr_dict,D=2,mode="ex",L_int_1=L_int_z,L_int_2 = L_int_z,F_dict = self.F_dict)
        ...
        for nothing in range(self.iterations-1):
            ...
            for msg_block_index in range(len(msg_block_L_list_1)-1):
                ...
                for j in range(msg_block_1.shape[1]):
                    ...
                    if (msg_block_index,j,1) in self.fixed_L_dict:
                        msg_block_L_list_1[msg_block_index][:,j] = self.fixed_L_dict[(msg_block_index,j,1)]
                        self.fin_msg_block_list_1[msg_block_index][:,j] = self.fixed_L_dict[(msg_block_index,j,1)]
                    else:
                        LDPC_ouput_1 = log_LDPC_decode(self.H_matrix,self.G_matrix,L_int=msg_block_1[:,j],round = nothing+1)
                        fin_column_L_array_1  = LDPC_ouput_1[0] 
                        column_L_array_1  = LDPC_ouput_1[1] 
                        fix_mode_1 = LDPC_ouput_1[3] 
                        if fix_mode_1 == 1:
                            self.fixed_L_dict[(msg_block_index,j,1)] = column_L_array_1
                        msg_block_L_list_1[msg_block_index][:,j] = column_L_array_1     
                        self.fin_msg_block_list_1[msg_block_index][:,j] = fin_column_L_array_1                   

                    if (msg_block_index,j,2) in self.fixed_L_dict:
                        msg_block_L_list_2[msg_block_index][:,j] = self.fixed_L_dict[(msg_block_index,j,2)]
                        self.fin_msg_block_list_2[msg_block_index][:,j] = self.fixed_L_dict[(msg_block_index,j,2)]
                    else:
                        LDPC_ouput_2 = log_LDPC_decode(self.H_matrix,self.G_matrix,L_int=msg_block_2[:,j],round = nothing+1)
                        fin_column_L_array_2 = LDPC_ouput_2[0]
                        column_L_array_2 = LDPC_ouput_2[1] 
                        fix_mode_2 = LDPC_ouput_2[3]
                        if fix_mode_2 == 1:
                            self.fixed_L_dict[(msg_block_index,j,2)] = column_L_array_2
                        msg_block_L_list_2[msg_block_index][:,j] = column_L_array_2
                        self.fin_msg_block_list_2[msg_block_index][:,j] = fin_column_L_array_2
    ...
    def Acc(self,name) :
        ...
        error_file = open("error_log"+str(name),"w")
        for k in range(len(msg_list)):

            msg_block = msg_list[k]
            fin_block = self.fin_msg_block_list[k]
            if k == 1612:
                acc_name = str(name)+"ACC"
                g = open(acc_name,"w")
                g.write(str(msg_block))
                g.write(str(fin_block))
            ...
            for i in range(msg_block.shape[0]):
                for j in range(msg_block.shape[1]):
                    ...
                    if msg_block[i,j] != fin_block[i,j]:
                        ...
                        now_error_list = [k,i,j,msg_block[i,j],fin_block[i,j]]
                        error_file.write(str(now_error_list))
                        error_bit += 1
        print("ACC:",(total_bit-error_bit)/total_bit,name)
        error_file.close()
